...
namespace MRQ.CryptoBot.Integration.Nethereum
{
    public class PancakeSwapAdapter : IBlockChainOperationAdapter
    {
        ...
        public async Task GetWalletBalanceOfTokenAsync(WalletDto walletDto, TokenDto tokenDto)
        {
            ...
            var web3 = new Web3(ConfigurationStaticDto.Web3Url);

            try
            {
                var contract = web3.Eth.GetContract(ConfigurationStaticDto.ContractABI, tokenDto.Adress);
        ...
        public async Task<Returned> SwapTokensAsync(WalletDto walletDto, TokenDto tokenOrigin, TokenDto tokenDestination)
        {
            ...
            try
            {
                var url = ConfigurationStaticDto.Web3Url;
                var myWallet = walletDto.Adress;
                var privateKey = walletDto.PrivateKey;
                var contractAddress = ConfigurationStaticDto.ContractRouterForSwap;
                ...
                var camtidadToken = Web3.Convert.ToWei(decimal.Parse(tokenDestination.Balance) * ConfigurationStaticDto.SlippageTolerance);
                ...
                var swapDTO = new SwapExactTokensForTokens()
                {
                    ...
                    GasPrice = ConfigurationStaticDto.DefaultGasPrice,
                    Gas = ConfigurationStaticDto.DefaultGas
                };
